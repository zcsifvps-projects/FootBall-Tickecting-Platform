...
// Helper to generate seat map from zone data
const generateSeatMap = (zones: any[]): Block[] => {
  return zones.map((zone, idx) => ({
    id: `ZONE-${idx}`,
    name: zone.name,
    wing: (zone.name || "") as any,
    gate: `Gate ${idx + 1}`,
    price: zone.price,
    rows: Array.from({ length: 8 }, (_, r) => ({
      rowNumber: r + 1,
      seats: Array.from({ length: 15 }, (_, s) => ({
        seatNumber: s + 1,
        status: Math.random() > 0.85 ? "sold" : ("available" as SeatStatus),
      })),
    })),
  }));
};
...
export default function MatchDetails() {
  ...
  // Generate seating blocks from match zones
  const stadiumMap = useMemo(() => {
    if (!match?.zones) return { blocks: [] };
    return { blocks: generateSeatMap(match.zones) };
  }, [match?.zones]);

  // Restore UI state if user already selected tickets for this match
  useEffect(() => {
    window.scrollTo(0, 0);

    if (!match) return;

    const existingItem = cart.find((item) => item.matchId === match._id);
    if (existingItem) {
      const block = stadiumMap.blocks.find((b) => b.name === existingItem.zone);
      if (block) {
  ...
  }, [match?._id, cart, stadiumMap.blocks]);

  const selectedBlock = useMemo(
    () => stadiumMap.blocks.find((b) => b.id === selectedBlockId) || null,
    [selectedBlockId, stadiumMap.blocks]
  );
  const selectedRow = useMemo(
    () => selectedBlock?.rows.find((r) => r.rowNumber === selectedRowNumber) || null,
  ...
  const handleAddToCart = () => {
    ...
    if (!isAuthenticated) {
      toast({
        ...
        description: "Please sign in before purchasing tickets.",
      });
    ...
    <div className="min-h-screen flex flex-col bg-[#FDFDFD] font-sans">
      ...
      {isLoading ? (
        ...
        <main className="flex-1 container mx-auto px-4 py-12 text-center">
          ...
          <p className="text-red-500 font-semibold">Error loading match</p>
          ...
          <div className="relative rounded-[2.5rem] overflow-hidden mb-10 shadow-2xl h-[400px]">
            ...
            <div className="relative z-20 p-8 md:p-12 h-full flex flex-col justify-end">
              ...
              <h1 className="text-5xl md:text-7xl font-bold text-white mb-4 tracking-tight drop-shadow-[0_2px_10px_rgba(0,0,0,0.5)]">
                {match.homeTeam} <span className="text-[#FF9900]">vs</span> {match.awayTeam}
              </h1>
              ...
              <div className="flex flex-wrap gap-8 text-white font-medium text-sm md:text-base drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)]">
                <span className="flex items-center gap-2">
                  <Calendar className="h-5 w-5 text-[#FF9900]" /> {match.date}
                </span>
                <span className="flex items-center gap-2">
                  <Clock className="h-5 w-5 text-[#FF9900]" /> {match.time}
                </span>
                <span className="flex items-center gap-2">
                  <MapPin className="h-5 w-5 text-[#FF9900]" /> {match.stadium || match.city}
                </span>
              </div>
            </div>
          </div>
                  ...
                  <CardContent className="p-0">
                    ...
                    <div className="bg-slate-50/50 p-4 grid grid-cols-2 md:grid-cols-5 gap-3 border-b border-slate-100">
                      {stadiumMap.blocks.map((b) => (
                        ...
                        >
                          <p
                            className={`font-black text-[10px] uppercase tracking-tight mb-0.5 ${
                              selectedBlockId === b.id ? "text-[#0e633d]" : "text-slate-500"
                            }`}
                          >
                            {b.wing}
                          </p>
                    ...
                    {selectedBlock ? (
                            ...
                            <div className="flex flex-wrap justify-center gap-3 mb-10">
                              {selectedBlock.rows.map((r) => (
                                <button
                                  key={r.rowNumber}
                                  onClick={() => {
                                    setSelectedRowNumber(r.rowNumber);
                                    setSelectedSeatNumbers([]);
                                  }}
                                ...
                                {selectedRow.seats.map((s) => {
                                  ...
                                  return (
                                    <button
                                      ...
                                      disabled={isSold}
                                      onClick={() => {
                                        const newSeats = selectedSeatNumbers.includes(s.seatNumber)
                                          ? selectedSeatNumbers.filter((n) => n !== s.seatNumber)
                                          : [...selectedSeatNumbers, s.seatNumber].sort((a, b) => a - b);
                                        setSelectedSeatNumbers(newSeats);
                                      }}
                      ...
                      <div className="py-20 text-center bg-white">
                        <img
                          ...
                          className="w-16 h-10 mx-auto mb-4 opacity-20 object-cover rounded-md"
                          alt="Zambia"
                        />
                        <h3 className="text-xl font-bold text-[#0e633d] uppercase">Select a Zone</h3>
